...
from healing.openstack.common.db import exception
from healing.openstack.common.gettextutils import _LE, _LW
    ...
    def _do_query(self, q):
        ...
        for filename, line, method, function in traceback.extract_stack():
            ...
            if filename.endswith('db/api.py'):
                continue
            # only trace inside healing
            index = filename.rfind('healing')
            if index == -1:
                continue
...
class EngineFacade(object):
    """A helper class for removing of global engine instances from healing.db.

    As a library, healing.db can't decide where to store/when to create engine
    and sessionmaker instances, so this must be left for a target application.

    On the other hand, in order to simplify the adoption of healing.db changes,
